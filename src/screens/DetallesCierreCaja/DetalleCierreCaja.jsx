...
const detalleDefault = {
  ...
  monto: 1.00,
  cantidad: 1,
  detalle_aperturacaja_idregistro: 1,
  denominacionmonedas_idregistro: 1
}
...
const campos = [
  ...
  "Monto",
  "Cantidad",
  "Id Detalle Apertura Caja",
  "Id Denominacion monedas",
]

export function DetalleCierreCaja({ navigation, route }) {
  ...
  async function crearDetalle() {
    try {
      const res = fetch(`${API_URL}/detallecierrecaja${route.params ? "/" + detalle.idregistro : ""}`, {
  ...
  async function eliminarDetalle() {
    try {
      await fetch(`${API_URL}/detallecierrecaja/${detalle.idregistro}`, {
        method: "DELETE",
      })
