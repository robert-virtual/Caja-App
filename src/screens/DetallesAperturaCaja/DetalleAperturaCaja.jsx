...
export function DetalleAperturaCaja({ navigation, route }) {
  ...
  async function crearDetalle() {
    try {
      const res = fetch(`${API_URL}/detalleaperturacaja${route.params ? "/" + detalle.idregistro : ""}`, {
  ...
  async function eliminarDetalle() {
    try {
      await fetch(`${API_URL}/detalleaperturacaja/${detalle.idregistro}`, {
        method: "DELETE",
      })
